...
from models.reseller import Reseller, ResellerApplication, CreditTransaction
...
async def create_licenses_bulk(
    ...
    if duration_days < 1 or duration_days > 36500:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Duration must be between 1 and 36500 days"
        )
    ...
    credit_cost = Decimal(str(duration_days * count))
    ...
    transaction = CreditTransaction(
        ...
        balance_after=new_balance,
        transaction_type="usage",
        description=f"Created {count} licenses for {app.name} ({duration_days} days each)"
    )
    db.add(transaction)
...
async def get_my_licenses(
    ...
    for lic in licenses:
        ...
        result.append({
            ...
            "hwid": lic.hwid,
            "expiry_timestamp": lic.expires_at.isoformat() if lic.expires_at else None,
...
async def delete_license(
    ...
    """Delete a license created by the reseller"""
    license = db.query(License).filter(
        License.id == license_id,
        License.created_by_reseller_id == current_reseller.id
    ).first()
...
async def reset_license_hwid(
    request_data: ResetHWIDRequest,
    current_reseller: Reseller = Depends(get_current_reseller),
    db: Session = Depends(get_db)
):
    """Reset HWID for a license"""
    license = db.query(License).filter(
        License.key == request_data.license_key,
        License.created_by_reseller_id == current_reseller.id
    ).first()
    ...
    # Clear HWID binding
    license.hwid = None
    license.hwid_hash = None
    license.hwid_bound_at = None
    db.commit()
    db.refresh(license)
