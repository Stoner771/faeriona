...
from controllers.reseller_controller import (
    ...
    get_reseller_applications, add_reseller_balance, deduct_reseller_balance
)
...
@router.post("/{reseller_id}/add-balance", response_model=CreditTransactionResponse)
async def add_balance_endpoint(
    reseller_id: int,
    data: BalanceRequest,
    current_admin: Admin = Depends(get_current_admin),
    db: Session = Depends(get_db)
    ...
    try:
        return add_reseller_balance(db, reseller_id, data.amount, current_admin.id, data.description)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))


@router.post("/{reseller_id}/deduct-balance", response_model=CreditTransactionResponse)
async def deduct_balance_endpoint(
    reseller_id: int,
    data: BalanceRequest,
    current_admin: Admin = Depends(get_current_admin),
    db: Session = Depends(get_db)
    ...
    try:
        return deduct_reseller_balance(db, reseller_id, data.amount, current_admin.id, data.description)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
