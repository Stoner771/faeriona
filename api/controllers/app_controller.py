...
def delete_app(db: Session, app_id: int, admin_id: int):
    ...
    from models.reseller import ResellerApplication
    
    # Delete licenses
    db.query(License).filter(License.app_id == app_id).delete(synchronize_session=False)
    
    # Delete variables
    db.query(Variable).filter(Variable.app_id == app_id).delete(synchronize_session=False)
    
    # Delete users
    db.query(User).filter(User.app_id == app_id).delete(synchronize_session=False)
    
    # Delete files
    db.query(File).filter(File.app_id == app_id).delete(synchronize_session=False)
    
    # Delete logs
    db.query(Log).filter(Log.app_id == app_id).delete(synchronize_session=False)
    
    # Delete reseller applications (junction table)
    db.query(ResellerApplication).filter(ResellerApplication.app_id == app_id).delete(synchronize_session=False)
